...
class UserModel{
    ...
    public function getAllUsers() {
      ...
      courriel = ?, mdp = ? WHERE id = ?");
        ...
        public function updateUserNom($id, $data) {
          $stmt = $this->pdo->prepare("UPDATE user SET nom = ? WHERE id = ?");
        
          return $stmt->execute([$data['nom'], $id]);
          }
          public function updateUserEmail($id, $data) {
            $stmt = $this->pdo->prepare("UPDATE user SET courriel = ? WHERE id = ?");
          
            return $stmt->execute([$data['courriel'], $id]);
            }
            public function updateUserPassword($id, $data) {
              $stmt = $this->pdo->prepare("UPDATE user SET mdp = ? WHERE id = ?");
            
              return $stmt->execute([$data['mdp'], $id]);
              }
